...
function MarqueeBanner({ direction, speed, flip }: {
  direction: 'left' | 'right'; speed: number; flip: boolean;
}) {
        ...
        >
          {[...Array(COPIES)].map((_, ci) => (
            <svg
              ...
              width={TEXT_WIDTH}
              height={HALF}
              viewBox={`0 ${viewBoxY} ${TEXT_WIDTH} ${HALF}`}
              style={{ flexShrink: 0 }}
            >
              <defs>
                <linearGradient id={`shine-${flip ? 'b' : 't'}-${track}-${ci}`}
                  gradientUnits="userSpaceOnUse" x1={`${-TEXT_WIDTH}`} y1="0" x2="0" y2="0">
              ...
              {/* Base dim outline */}
              <text x="20" y={SVG_FONT_SIZE + 6}
                fontFamily="'Inter','Helvetica Neue',Arial,sans-serif" fontWeight="900"
                fontSize={SVG_FONT_SIZE} letterSpacing="-1"
                fill="none" stroke="rgba(34,197,94,0.2)" strokeWidth="1.2">
                {MARQUEE_TEXT}
              </text>
              {/* Shimmering outline */}
              <text x="20" y={SVG_FONT_SIZE + 6}
                ...
                fontSize={SVG_FONT_SIZE} letterSpacing="-1"
                fill="none"
                stroke={`url(#shine-${flip ? 'b' : 't'}-${track}-${ci})`}
                strokeWidth="1.2"
                filter={`url(#glow-${flip ? 'b' : 't'}-${track}-${ci})`}>
                {MARQUEE_TEXT}
              </text>
            </svg>
          ))}
...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const [phase, setPhase] = useState(0); // 0=nothing, 1=letters, 2=tagline+form
  ...
  return (
    <div className="min-h-screen flex overflow-hidden relative"
      style={{ background: 'linear-gradient(135deg, #03100a 0%, #051a0c 50%, #041209 100%)' }}>

      {/* ── Half-visible scrolling marquee — top ────────────────────────────────── */}
      <MarqueeBanner direction="left" speed={28} flip={false} />

      {/* ── Half-visible scrolling marquee — bottom ─────────────────────────────── */}
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div className="absolute bottom-[-20%] right-[-8%] w-[60vw] h-[60vw] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(22,163,74,0.09) 0%, transparent 65%)' }} />
        <div className="absolute top-[40%] right-[25%] w-[25vw] h-[25vw] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(74,222,128,0.06) 0%, transparent 65%)' }} />
      </div>

      {/* ── LEFT PANEL — Animated VERDISORT ────────────────────────────────────── */}
      <div className="hidden lg:flex lg:w-[55%] flex-col justify-center pl-16 pr-8 relative"
        style={{ zIndex: 2 }}>
        <div>
          {/* ── Single-line VERDISORT with spring letter reveal ── */}
              ...
              return (
                <span
                  ...
                  style={{
                    ...
                    ...(isGreen ? {
                      background: 'linear-gradient(160deg, #4ade80 0%, #22c55e 55%, #15803d 100%)',
                      ...
                      textShadow: '0 0 50px rgba(34,197,94,0.07)',
                    }),
                    opacity: phase >= 1 ? 1 : 0,
          ...
          >
            <p className="text-gray-300 text-xl font-light leading-snug">
              ...
              <span style={{ color: '#4ade80', fontWeight: 500 }}>Intelligent Data Streams</span>
            </p>
          </div>
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center p-6 lg:p-12 relative"
        ...
        >
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(160deg, rgba(8,20,12,0.90) 0%, rgba(5,14,9,0.95) 100%)',
              backdropFilter: 'blur(36px)',
              borderColor: 'rgba(34,197,94,0.14)',
              boxShadow: '0 0 0 1px rgba(255,255,255,0.04) inset, 0 32px 64px rgba(0,0,0,0.55), 0 0 50px rgba(34,197,94,0.05)',
            }}
          >
            {/* Tab switcher */}
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              {mode === 'signup' && (
                <InputField id="name" label="Full Name" type="text" value={name}
                  onChange={setName} placeholder="Jane Doe" />
              )}
              <InputField id="email" label="Email" type="email" value={email}
                onChange={setEmail} placeholder="you@verdisort.ai"
              ...
              {mode === 'signup' && (
                <InputField id="confirm" label="Confirm Password" type="password" value={confirm}
          ...
          <p className="text-center text-xs mt-5" style={{ color: 'rgba(255,255,255,0.13)', letterSpacing: '0.15em' }}>
